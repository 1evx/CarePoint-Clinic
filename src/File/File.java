...
public class File {
    ...
    public static void readFile(Payments payments) {
        try (BufferedReader reader = new BufferedReader(new FileReader("Resource/payment.txt"))) {
            payments.addPaymentHeader(reader.readLine());
            for (String data = reader.readLine(); data != null; data = reader.readLine()) {
                String[] paymentInfo = data.split(",");
                PaymentData payment = new PaymentData(paymentInfo[0], paymentInfo[1], paymentInfo[2], paymentInfo[3], paymentInfo[4], paymentInfo[5], paymentInfo[6], paymentInfo[7], paymentInfo[8], paymentInfo[9], paymentInfo[10]);
